...
function PropertyCard({ property }: { property: any }) {
  return (
    <div className="bg-navy-800 rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 border border-navy-700 group">
      <div className="relative h-64 w-full overflow-hidden bg-navy-900 border-b border-navy-700">
        {property.imageUrl ? (
          <img
...
function PostCard({ post }: { post: any }) {
  ...
  return (
    <div className="bg-navy-800 rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 border border-navy-700 group flex flex-col h-full">
      <div className="relative h-48 w-full overflow-hidden bg-navy-900 border-b border-navy-700 shrink-0">
        {post.imageUrl ? (
          <img
            src={post.imageUrl}
            alt={post.title}
      ...
      <div className="p-6 flex flex-col grow">
        <div className="text-sm font-medium text-orange-400 mb-3">{formattedDate}</div>
        <h3 className="text-xl font-bold text-white mb-4 line-clamp-2">{post.title}</h3>
        <div className="mt-auto">
          <Link href={`/post/${post.slug}`} className="text-orange-400 font-bold hover:text-orange-300 inline-flex items-center gap-1 transition-colors">
            続きを読む
...
export default async function Home() {
  ...
  return (
    <>
      ...
      {/* Properties Section */}
      <section className="py-20 bg-navy-900 text-white container mx-auto px-6 lg:px-12">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">おすすめの物件</h2>
      ...
      <section className="py-20 bg-navy-900 text-white">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">学区・エリアマップ</h2>
            <p className="text-gray-300 max-w-2xl mx-auto text-lg">
          ...
          <div className="mt-20 max-w-4xl mx-auto">
            <h3 className="text-2xl md:text-3xl font-bold text-white mb-8 text-center flex items-center justify-center gap-3">
            ...
            <div className="bg-navy-800 rounded-3xl shadow-xl overflow-hidden border border-navy-700">
              {highSchools?.length > 0 ? highSchools.map((school: any, index: number) => (
                <div key={school._id} className={`flex items-start md:items-center p-6 sm:p-8 ${index !== highSchools.length - 1 ? 'border-b border-navy-700' : ''} hover:bg-navy-700/50 transition-colors`}>
                  <div className="flex-shrink-0 mr-6 text-center">
                    <span className={`inline-flex items-center justify-center w-12 h-12 rounded-full font-bold text-xl ${index === 0 ? 'bg-yellow-400 text-navy-900 shadow-md transform scale-110' : index === 1 ? 'bg-gray-300 text-navy-900 shadow-sm' : index === 2 ? 'bg-orange-400 text-navy-900 shadow-sm' : 'bg-navy-900 text-gray-400 border border-navy-700'}`}>
                      {school.rank || index + 1}
                    </span>
                  </div>
                  <div className="flex-grow">
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 mb-2">
                      ...
                      {school.deviationValue && (
                        <span className="inline-block bg-orange-500/20 text-orange-400 text-sm font-bold px-3 py-1 rounded-full whitespace-nowrap self-start md:self-auto border border-orange-500/30">
      ...
      < section className="py-24 bg-navy-900 text-white" >
        <div className="container mx-auto px-6 lg:px-12">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">福岡暮らしのコラム</h2>
            <p className="text-gray-300 max-w-2xl mx-auto text-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12 text-gray-400 bg-navy-800 rounded-2xl shadow-sm border border-navy-800">
